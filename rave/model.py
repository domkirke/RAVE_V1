...
class RAVE(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.pqmf is not None:
            x = torch.cat([self.pqmf(batch[:, np.newaxis, i]) for i in range(batch.shape[1])], 1)
            p.tick("pqmf")
        ...
        if self.pqmf is not None:
            x = x.view(x.shape[0], in_channels, -1, x.shape[-1])
            y = y.view(y.shape[0], in_channels, -1, y.shape[-1])
            x = torch.stack([self.pqmf.inverse(x[:, i]).squeeze(-2) for i in range(in_channels)], dim=1)
            y = torch.stack([self.pqmf.inverse(y[:, i]).squeeze(-2) for i in range(in_channels)], dim=1)
            distance = distance + self.distance(x, y)
            p.tick("fb distance")

        loud_x = sum([self.loudness(x[:, i].unsqueeze(1)) for i in range(in_channels)])
        loud_y = sum([self.loudness(y[:, i].unsqueeze(1)) for i in range(in_channels)])
    ...
    def decode(self, z):
        ...
        if self.pqmf is not None:
            y = y.view(y.shape[0], self.in_channels, -1, y.shape[-1])
            y = torch.stack([self.pqmf.inverse(y[:, i]).squeeze(-2) for i in range(self.in_channels)], dim=1)
        return y
