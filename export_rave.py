...
class TraceModel(nn.Module):
    def __init__(self, pretrained: RAVE, resample: Resampling,
        ...
        self.in_channels = pretrained.in_channels
        ...
        x = torch.zeros(1, self.in_channels, 2**14)
        z = self.encode(x)
        ...
        self.register_buffer("forward_params",
                             torch.tensor([self.in_channels, 1, self.in_channels, 1]))
    ...
    def encode(self, x):
        ...
        if self.pqmf is not None:
            x = torch.cat([self.pqmf(x[:, np.newaxis, i]) for i in range(x.shape[1])], 1)
    ...
    def decode(self, z):
        ...
        # if z.shape[0] == 1:
        #     z = z.expand(self.in_channels, z.shape[1], z.shape[2])
        ...
        if self.pqmf is not None:
            x = x.view(x.shape[0], self.in_channels, -1, x.shape[-1])
            x = torch.stack([self.pqmf.inverse(x[:, i]).squeeze(-2) for i in range(self.in_channels)], dim=1)

        x = torch.cat([self.resample.to_target_sampling_rate(x[:, i, :].unsqueeze(1)) for i in range(x.shape[-2])], -2)
        return x
...
x = torch.zeros(1, model.in_channels, 2**14)
if model.pqmf is not None:
    x = torch.cat([model.pqmf(x[:, np.newaxis, i]) for i in range(x.shape[1])], 1)

z, _ = model.reparametrize(*model.encoder(x))

z = z.expand(model.in_channels, *z.shape[1:])

y = model.decoder(z)

if model.pqmf is not None:
    y = y.view(y.shape[0], model.in_channels, -1, y.shape[-1])
    y = torch.stack([model.pqmf.inverse(y[:, i]).squeeze(-2) for i in range(model.in_channels)], dim=1)
